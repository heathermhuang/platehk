...
When a fresh exact signal exists, the buyer CTA opens a short form for the target plate, maximum budget, and an optional note. Until both `INTRODUCTION_SERVICE_URL` and `INTRODUCTION_WHATSAPP_NUMBER` are configured, submitting the form retains the existing client-side WhatsApp draft behavior. When enabled, the public market response supplies only the secondary introduction number, and the browser composes the structured WhatsApp draft locally; no form values are sent to Plate.hk or Render before the user presses Send in WhatsApp, and no buyer or seller contact is added to the 28car signal feed.

The isolated service under `services/whatsapp-introductions/` implements the HK$199 pilot. Buyer consent, seller consent, buyer approval of the specific self-identified seller, payment, group creation, buyer confirmation, refund, and retention are separate observable states. Its OpenWA browser session, proxy credentials, Stripe secrets, and encrypted state stay on Render and must not enter the public Cloudflare assets. See the service README and `render.yaml` for deployment gates.
